...
def generate_user_exe_from_database():
    try:
        ...
        query = "SELECT Username, Password, Email FROM hash"
        cursor.execute(query)
        user_data = cursor.fetchall()

        output_dir = os.path.join(os.getcwd(), "hash_user_exe_files")
...
def authenticate(username, password):
    if not username or not password:
        print("EMPTY")
    elif username == VALID_USERNAME and hash_password(password) == VALID_PASSWORD:
        print("SUCCESS")
    else:
        print("FAILURE")
...
"""
            ...
            temp_py_file = f"{username}_hash.py"
            ...
            print(f"{username}_hash.exe generated with user details.")
